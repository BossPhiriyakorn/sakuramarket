...
export function RecommendedProducts({
  ...
  return (
    <section className="min-w-0">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-thin scrollbar-thumb-pink-900/50 scrollbar-track-transparent min-w-0">
        {products.length === 0 ? (
          <p className="text-pink-400/60 text-sm py-4">ยังไม่มีสินค้าแนะนำ</p>
        ) : (
          products.map((p) => {
            ...
            return (
              <div
                ...
                className={`flex-shrink-0 w-[260px] sm:w-[280px] rounded-2xl overflow-hidden flex flex-col border border-pink-900/25 bg-slate-900/70 backdrop-blur-sm shadow-lg min-w-0 ${onProductClick ? "cursor-pointer hover:border-pink-800/40 active:scale-[0.99] transition-transform" : ""}`}
              >
                ...
                <div className="p-3.5 sm:p-3 flex-1 flex flex-col min-h-0 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                    ...
                    <span className="text-slate-400 text-xs">ขายแล้ว {(meta?.sold_count ?? 0).toLocaleString()}</span>
                  </div>
